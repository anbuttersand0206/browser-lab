...
- **ネットワーク学習コース** — ARP・TCP・DNS・DHCP・OSPFをパケットアニメーションで視覚的に理解できる環境
...
### ネットワーク学習コース — OSI モデルと送受信カプセル化の可視化

![ネットワーク学習コース — OSI モデル](documents/screenshots/network_dark.png)
...
40 種のアルゴリズムをステップ実行・速度調整しながら視覚的に学べます。
...
| データ構造 | 連結リスト、二分探索木（BST）、ハッシュテーブル（チェーン法） |
...
- **比較モード (Side-by-Side)** — 2つのアルゴリズムを同じデータで並列実行し、速さの違いを視覚的に比較する
- **サウンド化 (Sonification)** — ソートの比較・スワップ操作を音程にマッピングし、アルゴリズムの動きを耳で追える（Web Audio API 使用、外部依存なし）
- **日本語 / 英語** 切り替え対応
...
4. **配列の高階関数をマスターする** — map / filter / reduce の使い方
5. **クラスと継承を理解する** — 抽象クラス・ポリモーフィズム
6. **Expressでシンプルなサーバーを作る** — ルーティング・ミドルウェア・プレビュー確認
7. **Genericsで型安全な汎用コードを書く** — 型パラメータ・Result 型
8. **型ガードで安全に型を絞り込む** — typeof / instanceof / discriminated union
9. **Reactでタスク管理アプリを作る** — Vite + React / useState でインタラクティブ UI
...
5. **シェルスクリプト** — 変数・環境変数、条件分岐、ループ、実用スクリプト作成、引数・数値計算（全 6 ミッション）
6. **ネットワーク** — アクセスログ解析、hosts ファイル編集、HTTP ヘッダー抽出、Node.js URL API（全 4 ミッション）
...
#### アルゴリズム可視化コース
- [ ] **比較モード (Side-by-Side)** — 2つのアルゴリズムを同じデータで並列実行し、効率を視覚的に比較する機能

#### プログラミング学習コース
- [ ] **テスト実行の自動化** — Vitest 等を組み込み、課題がクリアされたか自動判定する機能
- [ ] **ステップ実行デバッガ** — WebContainers 上の実行をステップ単位で追跡・可視化する簡易デバッガ
- [ ] **Monaco Editor (VS Code) への移行** — より高度な補完、定義ジャンプ、マルチカーソル等の操作性向上

#### DB 学習コース
- [ ] **ビジュアルクエリビルダー** — ドラッグ＆ドロップで SQL を組み立てられる UI
- [ ] **SQL 実行計画のグラフィカル表示** — EXPLAIN ANALYZE の結果をツリー図として視覚化する機能

#### インフラ学習コース
- [ ] **Vim 練習モード** — ターミナル内エディタの基本操作（モード切替、保存等）のインタラクティブガイド
- [ ] **インフラ構成の自動可視化** — 現在のディレクトリ構造やプロセス状態をトポロジー図として自動生成する機能

### UX / パフォーマンス / 連携
- [ ] **IndexedDB によるマルチプロジェクト保存** — ファイル書き出し不要で、ブラウザ内に複数の作業状態を保存・切り替えできる機能
- [ ] **File System Access API 連携** — ブラウザ上の仮想環境をローカルの実際のフォルダと同期する機能
- [ ] **P2P 共同学習モード** — WebRTC を使い、サーバーを介さず他のユーザーと画面を共有・共同編集する機能

### メンテナンス / 品質
- [ ] **React 19 へのアップグレード** — 最新の React 機能への追従とパフォーマンス改善
- [ ] **Playwright による E2E テストの拡充** — 各学習コースの主要シナリオの自動テスト追加
