...
### 学習体験の強化
- [x] **シナリオ完了マーク** — 模範解答を確認表示したシナリオにサイドバーで ✓ を付ける（LocalStorage で永続化）
- [x] **コードリセットボタン** — ツールバーの「リセット」ボタンでシナリオ初期状態に戻す（確認ダイアログあり）
- [ ] **解答との差分表示** — 現在のコードと模範解答を並べて diff 表示し、どこが違うかを視覚的に示す
- [ ] **出力の自動採点** — 期待される出力パターンと実行結果を照合し、シナリオクリアを判定する

### エディタ・結果表示
- [x] **エディタ内検索** — `Ctrl/Cmd+F` でドキュメント内テキスト検索（`@codemirror/search` 拡張）
- [ ] **SQL フォーマッター** — キーワードの大文字統一・インデント整形をワンクリックで実行
- [x] **ResultGrid 列ソート** — 列ヘッダークリックで昇順/降順ソート（3クリック目でリセット）
- [x] **ResultGrid ページネーション** — 100 行超のとき前後ページボタンで分割表示

### シナリオ拡充
- [ ] **プログラミングコース** — Generics・型ガード（`is` 演算子）・デコレーター・イテレータ などのシナリオ追加
- [ ] **DB コース** — CTE（`WITH` 句）・`VIEW` の作成と活用・`CASE` 式などのシナリオ追加
- [ ] **シナリオ別進捗保存** — シナリオごとに編集内容を個別保存し、別のシナリオを触っても前の作業が消えないようにする（現在は最後の1シナリオのみ保存）
- [ ] **シナリオ URL 共有** — URL ハッシュにシナリオ ID を含めて直接リンクできるようにする（例: `/#/database/join-queries`）

### 品質・アクセシビリティ
- [ ] **PWA オフライン対応** — WebContainer / PGLite のバイナリも含めてキャッシュし、完全オフラインで動作可能にする
- [ ] **i18n 対応** — 多言語対応（英語/日本語）の基盤作成
- [ ] **Lighthouse 改善** — バンドル遅延読み込み（動的 `import()`）でパフォーマンスをさらに最適化
