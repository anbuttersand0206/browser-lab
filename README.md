...
#### DB 学習コース
- [x] **ER 図の自動生成** — 現在のスキーマから ER 図を動的に可視化する機能
- [ ] **ビジュアルクエリビルダー** — ドラッグ＆ドップで SQL を組み立てられる UI

#### アルゴリズム可視化コース
- [ ] **比較モード** — 2 つのアルゴリズムを同時に動かして速度や挙動を比較する機能
- [ ] **カスタム入力対応** — 任意の配列、グラフデータ、文字列を入力して可視化できる機能
- [ ] **データ構造ビジュアライザー** — 連結リスト、木構造（AVL, 赤黒木）、ハッシュテーブル等の挙動可視化

#### インフラ学習コース
- [ ] **プロセス管理ビジュアライザー** — 実行中のプロセスをツリー形式で可視化・操作できる機能
- [x] **ネットワーク構成図の可視化** — ミッションで扱うネットワーク構成を視覚的に表示する機能
- [ ] **Vim 練習モード** — ターミナル内エディタの基本操作（モード切替、保存等）のインタラクティブガイド

### UX / パフォーマンス / 連携
- [ ] **File System Access API 連携** — ブラウザ上の仮想環境をローカルの実際のフォルダと同期する機能
- [x] **学習レポート出力** — クリアしたミッションや書いたコードを Markdown / PDF で書き出す機能
- [ ] **P2P 共同学習モード** — WebRTC を使い、サーバーを介さず他のユーザーと画面を共有・共同編集する機能
- [x] **レスポンシブ対応の強化** — モバイル・タブレット端末でのレイアウト最適化
- [ ] **チュートリアルモード** — 初回訪問者向けに各機能の使い方を案内するツアー機能

### メンテナンス / 品質
- [ ] **E2E テストの拡充** — Playwright を使った主要シナリオの自動テスト追加
- [x] **アクセシビリティ (A11y) 対応** — スクリーンリーダー対応やキーボードナビゲーションの改善
