...
export function* ipv6Simulator(topology: Topology): NetworkStepGenerator {
  ...
  yield {
    state: empty(topology, [], 'compression'),
    log: {
  ...
  yield {
    state: empty(topology, [], 'header_fields'),
    log: {
  ...
  yield {
    state: empty(topology, [], 'transition'),
    log: {
